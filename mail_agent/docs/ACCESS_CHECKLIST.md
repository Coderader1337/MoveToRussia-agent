...
Ранее в git попали реальные секреты (см. [`STATUS.md`](STATUS.md) п.1). Файлы
`SUCCESS.md`, `START_HERE.md`, `.env.generated` **удалены** из репозитория;
...
- [ ] Ключ сгенерирован заново (см. предупреждение выше) и совпадает в обоих
      местах
...
[`STATUS.md`](STATUS.md) п.2). Для постоянного решения — VPS с домен/статик IP
вместо ngrok.
...
- [ ] Yandex Mail — все нужные ящики менеджеров, новые пароли приложений
- [ ] EnvyCRM — личный кабинет
- [ ] Google — аккаунт с таблицей "Neznajka" + Cloud-проект для `credentials.json`
- [ ] DeepSeek — личный кабинет, новый ключ
- [ ] Docker API — новый `API_KEY`
