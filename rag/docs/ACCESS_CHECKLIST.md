...
- [ ] **Telegram-бот (dev/test)** — то же самое для тестового бота, если он
  ...
  бесплатно и быстро).
- [ ] **Voyage AI** — есть доступ (email/пароль или совместный доступ) к
  аккаунту, на который зарегистрирован `VOYAGE_API_KEY`. Проверить: можно
  ли зайти и увидеть биллинг/usage без разработчика.
...
- `rag/bot/allowed_users.json` — не секрет в строгом смысле, но список
  приватных Telegram ID менеджеров — в git не коммитится по той же причине
  осторожности (проверить актуальный `.gitignore`, если нужно ужесточить).
